...
The script itself (`main.py`) fetches any new photos from a Google Drive folder, writes them to `images/`, and then shows them in a slideshow forever. There are instructions and a script (`run.sh`) for Linux to run the script on boot.
...
5. `python drive_api.py` to go through the auth process, save the token pickle file, and list accessible folders
6. Transfer `token.pickle` and `credentials.json` to the pi's `digital-frame` directory; set the folder ID in `main.py` on the pi (you can also get the folder ID from the Google Drive URL: `https://drive.google.com/drive/folders/YOUR_FOLDER_ID`).
